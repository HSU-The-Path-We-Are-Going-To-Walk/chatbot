...
from dto import ChatRequest, GeneralResponse, LocationInfo, PathInfo, BusInfo
...
@app.post(
    "/chat", response_model=Union[GeneralResponse, LocationInfo, PathInfo, BusInfo]
)
async def chat(request: ChatRequest):
    try:
        ...
        # 의도 처리는 동일하게 진행
        intent, destination = (
            chatbot.intent_processor.detect_intent_and_extract_destination(
                request.message
            )
        )
        ...
        if not intent:
            ...
            return GeneralResponse(response=response, success=True)
        ...
        # 길찾기
            ...
            formatted_result = chatbot.path_finder.format_path_result(
                route_data, place_name
            )

            conversation = PromptManager.convert_to_conversation(
                formatted_result["routes_text"], "경로"
            )
